...
void ClipboardGrabber::check_clipboard() {
...
void ClipboardGrabber::on_subject_changed(const QString &text) {
    ...
    if (!text.isEmpty()) {
        ...
        last_date_ = MarkdownUtils::restore_state_from_file(target_file);
...
void ClipboardGrabber::add_section() {
    ...
    QString text = QInputDialog::getText(this, "বিভাগ যোগ করুন",
                                         ...
                                         "", &ok);
    if (ok && !text.isEmpty()) {
        QString slug = QString::fromStdString(MarkdownUtils::generate_slug(text));
...
void ClipboardGrabber::shift_selected_heading_section() {
    ...
    if (MarkdownUtils::get_heading_bounds(content, source_slug, src_start, src_end, is_html)) {
        found = true;
    ...
    } else if (MarkdownUtils::get_subheading_bounds(content, source_slug, src_start, src_end)) {
        found = true;
    ...
    if (target_slug.isEmpty()) {
        ...
        bool tgt_found_h2 = MarkdownUtils::get_heading_bounds(content, target_slug, tgt_start, tgt_end, tgt_is_html);
        if (tgt_found_h2) {
            insert_pos = tgt_end;
            debugLog(QString("shift_selected_heading_section: target is H2, insert_pos=%1").arg(insert_pos));
        } else {
            bool tgt_found_h3 = MarkdownUtils::get_subheading_insert_pos(content, target_slug, insert_pos);
            if (!tgt_found_h3) {
...
void ClipboardGrabber::normalize_markdown_file(const QString &file_path) {
...
void ClipboardGrabber::write_image_to_file(const QString &image_filename) {
    QString target_file = get_current_target_file();
    if (target_file == "নির্বাচিত নয়") return;
...
void ClipboardGrabber::update_toc_in_file(const QString &file_path) {
    QFile file(file_path);
    if (!file.open(QIODevice::ReadWrite | QIODevice::Text)) {
        return;
    }
...
void ClipboardGrabber::parse_note_structure(const QString &file_path, QList<NoteItem> &items) {
    items.clear();
    QFile file(file_path);
...
void ClipboardGrabber::populate_headings_from_file() {
    ...
    MarkdownUtils::save_tree_file(target_file, all_headings_);
...
bool ClipboardGrabber::append_content_to_heading(const QString &file_path, const QString &slug, const QString &processed_text) {
    QFile file(file_path);
    if (!file.open(QIODevice::ReadWrite | QIODevice::Text)) {
        return false;
    }
    ...
    if (MarkdownUtils::get_heading_bounds(content, slug, start_pos, end_pos, is_html)) {
        ...
        int format_index = format_dropdown_->currentIndex();
        if (format_index == 2) {
            QString sub_slug = QString::fromStdString(MarkdownUtils::generate_slug(processed_text));
            ...
            QString main_slug = QString::fromStdString(MarkdownUtils::generate_slug(processed_text));
            QString section = section_dropdown_->currentData().toString();
...
void ClipboardGrabber::init_shortcut_configs() {
    shortcut_configs_ = {
...
void ClipboardGrabber::load_settings() {
...
void ClipboardGrabber::save_settings() {
    QSettings settings(notes_dir_path_ + QDir::separator() + "settings.ini", QSettings::IniFormat);
    settings.beginGroup("Shortcuts");
...
void ClipboardGrabber::setup_shortcuts() {
